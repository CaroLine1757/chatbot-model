...
def build_survey_prompt(survey_inputs):
    ...
    return (
        ...
        "- Pay attention to this instruction. Ask only one question at a time. This is a STRICT rule.\n"
...
def generate_survey_question(history, survey_prompt):
    """Bot 1: Generate a survey question with rate limit handling."""
...
def respond_to_question(question, history):
    """Bot 2: Respond to the survey question."""
    response = handle_rate_limit(
        openai.ChatCompletion.create,
        model="gpt-3.5-turbo",
        messages=[
